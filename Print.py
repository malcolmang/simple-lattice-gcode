...
class Print:
    def __init__(self, bounds, spacing, rotation, zheight, zspacing, feed_rate):
    ...
    def generate_savestring(self):
        return f'D{self.spacing}H{self.zheight}F{self.feed_rate}R{self.rotation}'
...
M84"""
        ...
        if print:
            ...
            with open(Path("gcode").joinpath(f"{self.savestring}.txt"), "w") as f:
                # Writing data to a file
                f.write(bufferstring)
            return f"Wrote successfully to gcode/{self.savestring}.txt!"
        return bufferstring
